...
# BCGRP and BCWAR are very similar, so many things are taken from unpack.BCGRP
# Actually, BCWAR is an archive of CWAV files while BCGRP is for any related audio files.

class extractBCWAR (rawutil.TypeReader):
	...
	def read_header(self, data):
		...
		if magic != b'CWAR':
			error.InvalidMagicError('Invalid magic %s, expected CWAR' % byterepr(magic))
	...
	def readINFO(self, data, start, size):
		...
		entrycount, table = self.unpack_from('I/p1[2H2I]', data)
		if self.verbose:
			print('Number of files: %d' % entrycount)
		self.fileinfo = []
		for id, pad, pos, size in table:
			if id == 0x1f00:  # File reference
				self.fileinfo.append({'pos': pos, 'size': size})
			else:
