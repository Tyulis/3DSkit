...
class extractBCLYT(ClsFunc, rawutil.TypeReader):
	...
	def parsedata(self):
		...
		self.tree['BCLYT'] = OrderedDict()
		self.actnode = self.tree['BCLYT']  # creates a pointer in the tree, which can change later
	...
	def readmat1(self, data):
		...
		ptr = 8
		matnum, ptr = self.uint32(data, ptr)
		...
		for offset in offsets:
			ptr = offset
			mat = OrderedDict()
			mat['name'] = self.string(data, ptr)[0]; ptr += 20
			mat['TEV-color'], ptr = self.color(data, ptr, 'RGBA8')
			mat['fix-colors'] = []
			for i in range(0, 6):
				mat['fix-colors'].append(self.color(data, ptr, 'RGBA8')[0]); ptr += 4
	...
	def readpane(self, data, ptr):
		...
		flags, ptr = self.uint8(data, ptr)
		node['ignore-parts-magnify'] = flags & 0b00000001
		node['adjust-to-parts-bounds'] = flags & 0b00000010
		node['name'] = self.string(data, ptr)[0]; ptr += 24
	...
	def readprt1(self, data):
		...
		for i in range(0, count):
			...
			self.actnode = entry
			self.actnode['__parentnode'] = parentnode
			entry['name'] = self.string(data, ptr)[0]; ptr += 24
			entry['unknown-1'], ptr = self.uint8(data, ptr)
			entry['flags'], ptr = self.uint8(data, ptr)
			pad, ptr = self.uint16(data, ptr)
			entryoffset, ptr = self.uint32(data, ptr)
			extraoffset, ptr = self.uint32(data, ptr)
			pad, ptr = self.uint32(data, ptr)
			if entryoffset != 0:
	...
	def readgrp1(self, data):
		ptr = 8
		name = self.string(data, ptr)[0]; ptr += 16
		secname = 'grp1-%s' % name
		self.actnode[secname] = OrderedDict()
		localnode = self.actnode[secname]
		localnode['name'] = name
		...
		for i in range(0, subnum):
			subs.append(self.string(data, ptr)[0]); ptr += 16
		localnode['subs'] = subs
